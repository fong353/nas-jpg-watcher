...
**仓库已转为结论文档，代码全部退役删除。** 面向外部读者的双语结论见 [`README.md`](./README.md)；
本文件保留为**完整工程笔记 + 故障模式分析 + 实测验证 + 联网核实记录**（中文，给后续维护者）。
退役的 Win 端实现（`scan.ps1` / `scan.cmd` / exiftool 整包等）仍可从 git 历史取回。

> 历史背景：本仓库原是一台 Win 机上的定时任务（每 30 min 扫 NAS、对齐 JFIF DPI），
> 后定时巡检迁到其他端，现连代码都不再需要——真正有价值的是下面这套**关于图像软件
> 如何解读 JPEG DPI 元数据的结论**，故开源留档。架构 / 文件职责等小节描述的是已删除的旧实现，仅供理解设计脉络。
...
## 2026-05-24 NAS 实测分布（推翻早期推断，必读）

对 `\\192.168.0.150\nas` 下最近 30 天的 `$Roots` 跑只读 audit（脚本：`audit.ps1`，结果：`audit.json`），命中 `JFIF.unit=None` 的文件 **63 个**，三段 XRes 交叉分布：

| 组合              | 数量 | 占比 |
|---|---|---|
| EXIF + JFIF (XRes>1) | 56  | 89% |
| 三段全无可信值     | 7   | 11% |
| Photoshop 8BIM    | **0** | **0%** |

**关键修正**：早期文档（设计决策、故障模式表）多次假设"业务员 PS Save 是主力路径、8BIM 几乎总在"——**实测推翻**：0/63 走 PS 路径。NAS 上坏文件的真实来源是手机/微信/修图 app/格式转换器等中间工具（保留 EXIF，不写 8BIM，错误地把 JFIF.unit 写成 None）。

**推论**：中间件本质是**单方向修复 ErgoSoft 侧**。PS 因为读取优先级里 EXIF > JFIF（unit=None 时 JFIF 不被信任），一直能从 EXIF 拿到正确尺寸——业务员在 PS 里看到的从来都是对的。两边分歧的根因只在 ErgoSoft：不读 EXIF + JFIF.unit=None 让它回落到默认 72。这也解释了为什么这个问题之前一直藏着——业务员发现不了，只有印刷端 ErgoSoft 暴露出来。

**端到端验证**：把 56 个 EXIF+JFIF 文件复制到桌面 `nas-jpg-audit-2026-05-24/fixed/` 跑修复，PS 和 ErgoSoft 同时打开对比，用户确认"都对齐了"。7 个"全无"文件按设计跳过（不动），PS 和 Ergo 都显示 72，本来就对齐。
